...
bool checkInputline(int index) {

    for (int j = 0; j < 8; j++) {
      if (inputSequence[index][j] != correctSequence[index][j]){
        return false;
      }
    }
...
void resetLine(int index) {
    for (int j = 0; j < 8; j++) {
      inputSequence[index][j] = 0; // Clear the input sequence
    }
  ...
  Serial.print("Error in the line");
  Serial.print(" retry ");
...
//         Serial.println("1");
      ...
      if (currentIndex == 8) {
        ...
        // }
        currentIndex = 0;
        if(checkInputline(currentArray))
        {
          Serial.print(currentArray+1);
          Serial.print("/5 ✓\n");
          currentArray++;
